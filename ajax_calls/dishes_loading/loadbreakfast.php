    ...
    if(isset($_POST['where_clause']) && $_POST['where_clause'] !=""){
        $where_clause = $_POST['where_clause'];
           ...
           if($menu_item_type=="both"){
                $_select_able_items_query ="SELECT `menu_item_id`, `menu_item_name`,`menu_item_feature_img`, `menu_item_price`, `menu_item_type`,`is_chef_special` FROM `menu_items` INNER JOIN menu_categories ON menu_items.menu_item_category=menu_categories.menu_c_id WHERE menu_categories.menu_c_name = 'breakfast'  AND (`menu_item_name` LIKE '%{$where_clause}%' OR `menu_item_price` LIKE '%{$where_clause}%');";
           }else{
                    $_select_able_items_query ="SELECT `menu_item_id`, `menu_item_name`,`menu_item_feature_img`, `menu_item_price`, `menu_item_type`,`is_chef_special` FROM `menu_items` INNER JOIN menu_categories ON menu_items.menu_item_category=menu_categories.menu_c_id WHERE menu_categories.menu_c_name = 'breakfast'  AND (`menu_item_name` LIKE '%{$where_clause}%' OR `menu_item_price` LIKE '%{$where_clause}%') AND menu_items.menu_item_type='{$menu_item_type}';";
           }
    }else{
           if($menu_item_type=="both"){
                $_select_able_items_query ="SELECT `menu_item_id`, `menu_item_name`,`menu_item_feature_img`, `menu_item_price`, `menu_item_type`,`is_chef_special` FROM `menu_items` INNER JOIN menu_categories ON menu_items.menu_item_category=menu_categories.menu_c_id WHERE menu_categories.menu_c_name = 'breakfast' ;";
           }else{
                $_select_able_items_query ="SELECT `menu_item_id`, `menu_item_name`,`menu_item_feature_img`, `menu_item_price`, `menu_item_type`,`is_chef_special` FROM `menu_items` INNER JOIN menu_categories ON menu_items.menu_item_category=menu_categories.menu_c_id WHERE menu_categories.menu_c_name = 'breakfast' AND menu_items.menu_item_type='{$menu_item_type}';";
           }
    }
