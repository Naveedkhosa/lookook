...
if (isset($_POST['item_type']) && isset($_POST['start'])) {
    ...
    if (isset($_POST['search']) && $_POST['search'] != "") {
        ...
        if ($menu_item_type == "both") {
            if ($start == 0) {
                $total_count_query = "SELECT `menu_item_id`, `menu_item_name`,`menu_item_feature_img`, `menu_item_price`, `menu_item_type`,`is_chef_special` FROM `menu_items` INNER JOIN menu_categories ON menu_items.menu_item_category=menu_categories.menu_c_id WHERE (menu_categories.menu_c_name = 'soups' OR  menu_categories.menu_c_name = 'beverages') AND `menu_item_name` LIKE '%{$search_term}%';";
                $total_count_result = mysqli_query($conn,$total_count_query);
                $result['total'] = mysqli_num_rows($total_count_result);
            }
            $select_query = "SELECT `menu_item_id`, `menu_item_name`,`menu_item_feature_img`, `menu_item_price`, `menu_item_type`,`is_chef_special` FROM `menu_items` INNER JOIN menu_categories ON menu_items.menu_item_category=menu_categories.menu_c_id WHERE (menu_categories.menu_c_name = 'soups' OR  menu_categories.menu_c_name = 'beverages') AND `menu_item_name` LIKE '%{$search_term}%' LIMIT $start, $limit;";
        } else {
            if ($start == 0) {
                $total_count_query = "SELECT `menu_item_id`, `menu_item_name`,`menu_item_feature_img`, `menu_item_price`, `menu_item_type`,`is_chef_special` FROM `menu_items` INNER JOIN menu_categories ON menu_items.menu_item_category=menu_categories.menu_c_id WHERE  (menu_categories.menu_c_name = 'soups' OR  menu_categories.menu_c_name = 'beverages') AND `menu_item_name` LIKE '%{$search_term}%' AND menu_items.menu_item_type='{$menu_item_type}';";
                $total_count_result = mysqli_query($conn,$total_count_query);
                $result['total'] = mysqli_num_rows($total_count_result);
            }
            $select_query = "SELECT `menu_item_id`, `menu_item_name`,`menu_item_feature_img`, `menu_item_price`, `menu_item_type`,`is_chef_special` FROM `menu_items` INNER JOIN menu_categories ON menu_items.menu_item_category=menu_categories.menu_c_id WHERE  (menu_categories.menu_c_name = 'soups' OR  menu_categories.menu_c_name = 'beverages') AND `menu_item_name` LIKE '%{$search_term}%' AND menu_items.menu_item_type='{$menu_item_type}' LIMIT $start, $limit;";
        ...
        if ($menu_item_type == "both") {
            ...
            $select_query = "SELECT `menu_item_id`, `menu_item_name`,`menu_item_feature_img`, `menu_item_price`, `menu_item_type`,`is_chef_special` FROM `menu_items` INNER JOIN menu_categories ON menu_items.menu_item_category=menu_categories.menu_c_id WHERE (menu_categories.menu_c_name = 'soups' OR  menu_categories.menu_c_name = 'beverages') LIMIT $start, $limit;";
        } else {
            if ($start == 0) {
