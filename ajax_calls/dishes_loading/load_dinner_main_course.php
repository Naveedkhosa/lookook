...
if (isset($_POST['_selected_items1'])) {
    $menu = $_POST['menu'];
    $choosed = explode(",", $menu);
    if($choosed[1]=="Italian-American"){
        $where_clause_is = "(menu_categories.menu_c_name = 'Italian' OR menu_categories.menu_c_name = 'American' ";
    }else{
        $where_clause_is = "(menu_categories.menu_c_name = '{$choosed[1]}' ";
    }
    for ($i = 2; $i < count($choosed); $i++) {
        if($choosed[$i]=="Italian-American"){
            $where_clause_is .= "OR menu_categories.menu_c_name = 'Italian' OR menu_categories.menu_c_name = 'American' ";
        }else{
            $where_clause_is .= "OR menu_categories.menu_c_name = '{$choosed[$i]}'";
        }
    ...
    if (isset($_POST['where_clause']) && $_POST['where_clause'] != "") {
        $where_clause = $_POST['where_clause'];
           if($menu_item_type=="both"){
                $_select_able_items_query = "SELECT `menu_item_id`, `menu_item_name`,`menu_item_feature_img`, `menu_item_price`, `menu_item_type`,`is_chef_special` FROM `menu_items` INNER JOIN menu_categories ON menu_items.menu_item_category=menu_categories.menu_c_id WHERE {$where_clause_is} AND menu_items.menu_item_appetizer='main_course' AND (`menu_item_name` LIKE '%{$where_clause}%' OR `menu_item_price` LIKE '%{$where_clause}%');";
           }else{
                $_select_able_items_query = "SELECT `menu_item_id`, `menu_item_name`,`menu_item_feature_img`, `menu_item_price`, `menu_item_type`,`is_chef_special` FROM `menu_items` INNER JOIN menu_categories ON menu_items.menu_item_category=menu_categories.menu_c_id WHERE {$where_clause_is} AND menu_items.menu_item_appetizer='main_course' AND menu_items.menu_item_type='{$menu_item_type}' AND (`menu_item_name` LIKE '%{$where_clause}%' OR `menu_item_price` LIKE '%{$where_clause}%');";
           }
    } else {
        
           if($menu_item_type=="both"){
                $_select_able_items_query = "SELECT `menu_item_id`, `menu_item_name`,`menu_item_feature_img`, `menu_item_price`, `menu_item_type`,`is_chef_special` FROM `menu_items` INNER JOIN menu_categories ON menu_items.menu_item_category=menu_categories.menu_c_id WHERE {$where_clause_is} AND menu_items.menu_item_appetizer='main_course';";
           }else{
                $_select_able_items_query = "SELECT `menu_item_id`, `menu_item_name`,`menu_item_feature_img`, `menu_item_price`, `menu_item_type`,`is_chef_special` FROM `menu_items` INNER JOIN menu_categories ON menu_items.menu_item_category=menu_categories.menu_c_id WHERE {$where_clause_is} AND menu_items.menu_item_appetizer='main_course' AND menu_items.menu_item_type='{$menu_item_type}';";
           }
    }
            ...
            while ($_select_able_items_row = mysqli_fetch_assoc($_select_able_items_result)) {
                ...
                if ($_selected_items_array != false) {
                    if (in_array($_select_able_items_row['menu_item_id'], $_selected_items_array)) {
                        ...
                        $_select_able_items_output .= '<div class="_select_able_items_ _select_able_items__32" id="khan_' . $_select_able_items_row['menu_item_id'] . '">
                            <div class="_img">
